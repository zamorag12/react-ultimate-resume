...
var DreamJobValidationSchema = function DreamJobValidationSchema(formatMessage) {
    return Yup.object({
        ...
        contractTypes: Yup.array()
            .test(
                'is-not-empty',
                formatMessage(_validator_translations.dreamjobValidationTranslations.atLeastOneContractType),
                function (value) {
                    return !!(value || []).length;
                }
            )
            .test(
                'is-exclusif',
                formatMessage(_validator_translations.dreamjobValidationTranslations.selectByGroup),
                function (value) {
                    return !(
                        ['permanent', 'fixedTerm', 'freelance'].filter(function (val) {
                            return value.includes(val);
                        }).length &&
                        ['apprenticeship', 'internship'].filter(function (val) {
                            return value.includes(val);
                        }).length
                    );
                }
            ),
        salary: Yup.number().min(
            1,
            formatMessage(_validation_translations.validationTranslations.min, {
                min: 1
            })
